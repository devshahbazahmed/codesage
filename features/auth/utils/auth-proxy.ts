...
// "/sign-in": If user is logged-in then user will redirect away from here. Only guest users are allowed
export async function handleAuthProxy(request: NextRequest) {
  ...
  if (pathname === SIGN_IN_PATH) {
    if (session) {
      const redirectPath = await getPostAuthRedirectPath(request);
